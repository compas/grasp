...
!     Modified by Chongyang Chen                                2023   *
!     Some minor fixes labelled by CYC-2023                            *
!     Modified by G. Gaigalas                                   2024   *
!                                                                      *
!***********************************************************************
!-----------------------------------------------
!   M o d u l e s
...
!
      integer, private :: IMINCOMPOFF, LS_convention
...
!CYC-2023 Corrections, perform transfermation for both even and odd
!states
            !.and.  ISPAR(iw1) == 1)  .or.                              &
            .and.  ISPAR(NCFMIN+iw1-1) == 1)  .or.                     &
            (asf_set_LS%csf_set_LS%csf(LS_number)%parity  == "-"       &
            !.and.  ISPAR(iw1) == -1)) then
            .and.  ISPAR(NCFMIN+iw1-1) == -1)) then
!CYC-2023 end
            wa = zero
               ...
               if(ithresh(jj_number) == 1 .and.                        &
                 (asf_set_LS%csf_set_LS%csf(LS_number)%totalJ ==       &
                  ...
                  do lev = 1,levmax
                     ...
                     LOC_i8 = (level-1)*NCF_i8
                     wa(lev) =                                         &
                      wa(lev)+EVEC(jj_number+LOC_i8)*wa_transformation
                  end do
               end if
            end do
            IMINCOMP = 1
...
!***********************************************************************
!                                                                      *
      SUBROUTINE getchLS_lbl(csf_number,shell_number,string_LS,string_XLS)
!                                                                      *
!     The spectroscopic notation of a shell in LS coupling is returned.*
!                                                                      *
!     Calls: convrt_double.                                            *
...
!   L o c a l   V a r i a b l e s
!-----------------------------------------------
      integer          :: string_lenth, w, TL, TS, seniority
      character(len=1) :: string_S, string_v
      character(len=4) :: string_CNUM
!-----------------------------------------------
      call                                                            &
      ...
      string_S = string_CNUM(1:string_lenth)
!GG-2024
      if (asf_set_LS%csf_set_LS%shell(shell_number)%l == 2)    then
         if (LS_convention == 0) THEN
            call                                                       &
                convrt_double(2*asf_set_LS%csf_set_LS%csf(csf_number)% &
                seniority(shell_number),string_CNUM,string_lenth)
            string_v = string_CNUM(1:string_lenth)
         else
           TS=asf_set_LS%csf_set_LS%csf(csf_number)%shellS(shell_number)
           TL=asf_set_LS%csf_set_LS%csf(csf_number)%shellL(shell_number)
           seniority=asf_set_LS%csf_set_LS%csf(csf_number)%seniority(shell_number)
           if(TS+1 == 1) then
              if(TL == 0) then
! 1S
                 if(seniority == 0) then
                    w = 1
                 else if(seniority == 4) then
                    w = 2
                 else
                    stop "getchLS_lbl(): program stop 1S."
                 end if
              else if(TL == 4) then
! 1D
                 if(seniority == 2) then
                    ...
                    stop "getchLS_lbl(): program stop 1D."
                 end if
              else if(TL == 8) then
! 1G
                 if(seniority == 2) then
                    ...
                    stop "getchLS_lbl(): program stop 1G."
                 end if
              else
                 w = 0
              end if
...
! 2D
                 if(seniority == 1) then
                    w = 1
                 else if(seniority == 3) then
                    w = 2
                 else if(seniority == 5) then
                    w = 3
                 else
                    stop "getchLS_lbl(): program stop 2D."
                 end if
              else if(TL == 6) then
! 2F
                 if(seniority == 3) then
                    ...
                    stop "getchLS_lbl(): program stop 2F."
                 end if
              else if(TL == 8) then
! 2G
                 if(seniority == 3) then
                    ...
                    stop "getchLS_lbl(): program stop 2G."
                 end if
              else
                 w = 0
              end if
...
! 3P
                 if(seniority == 2) then
                    ...
                    stop "getchLS_lbl(): program stop 3P."
                 end if
              else if(TL == 6) then
! 3F
                 if(seniority == 2) then
                    ...
                    stop "getchLS_lbl(): program stop 3F."
                 end if
              else
                 w = 0
              end if
           ...
           call convrt_double(2*w,string_CNUM,string_lenth)
           string_v = string_CNUM(1:string_lenth)
         end if 
      else if (asf_set_LS%csf_set_LS%shell(shell_number)%l < 3)    then
         if (LS_convention == 0) THEN
            call                                                       &
                convrt_double(2*asf_set_LS%csf_set_LS%csf(csf_number)% &
                seniority(shell_number),string_CNUM,string_lenth)
            ...
            w = asf_set_LS%csf_set_LS%csf(csf_number)%w(shell_number)
            call convrt_double(2*w,string_CNUM,string_lenth)
            string_v = string_CNUM(1:string_lenth)
         end if 
      else
      ...
      if(asf_set_LS%csf_set_LS%shell(shell_number)%l < 2) then
         string_LS = string_S //                                      &
         L_string(asf_set_LS%csf_set_LS%csf(csf_number)%              &
                                              shellL(shell_number)/2)
      else if(asf_set_LS%csf_set_LS%shell(shell_number)%l == 3        &
                                                   .AND. w == 0) then
         string_LS = string_S //                                      &
         L_string(asf_set_LS%csf_set_LS%csf(csf_number)%              &
                                              shellL(shell_number)/2)
      else if(LS_convention == 1 .AND. w == 0) then
         string_LS = string_S //                                      &
         L_string(asf_set_LS%csf_set_LS%csf(csf_number)%              &
                                              shellL(shell_number)/2)
      else
         if(w == 10) then
            string_LS = string_S //                                   &
            L_string(asf_set_LS%csf_set_LS%csf(csf_number)%shellL(    &
            shell_number)/2)//'A'
         else
            string_LS = string_S //                                   &
            L_string(asf_set_LS%csf_set_LS%csf(csf_number)%shellL(    &
            shell_number)/2)//string_v
         end if
      end if
!GG-2024 end
      !
...
!CYC-2023
! Modifications for configurations with empty 1s subshell
!                        Jcoup(JNW,JNCF)   = JQS(3,JNW,JNCF) - 1
                        Jcoup(JNW,JNCF)   = iabs(JCUP(JCNT,JNCF) - 2)
!CYC-2023 end
...
!GG-2024
         WRITE (ISTDE,*) '(default setting: 1.000)'
!GG-2024 end
         READ *, MINCOMP
...
!GG-2024
         WRITE (ISTDE,*) '(default setting: 0.005)'
!GG-2024 end
            READ *, EPSNEW
         ELSE
...
!GG-2024
         WRITE (ISTDE,*) '(default setting: 0.001)'
!GG-2024 end
         READ *, THRESH
         IF( MINCOMP > ZERO) THEN
...
!GG-2024
         WRITE (ISTDE,*)                                               &
         "Which convention should be used for d shell identification?" 
         WRITE (ISTDE,*)                                               &
           "Seniority notation / Nielson-Koster indexes (0/1)"
         READ *, LS_convention
!GG-2024 end
      ENDIF
!
...
!GG-2024
      ...
      IF (LS_convention == 0) THEN
        WRITE (*,"(A)")                                                &
            " Seniority notaton is used for d shell identification in"
        WRITE (*,"(5A)")"   ",NAME(1:K-1)//'.lsj'//'.lbl', ", ",       &
                              NAME(1:K-1)//'.uni.lsj'//'.lbl',","
        WRITE (*,"(5A)")"   ",NAME(1:K-1)//'.uni.lsj'//'.sum',", and ",&
                              NAME(1:K-1)//'.lsj'//'.j',"  files"
      ELSE
        WRITE (*,"(A)")                                                &
         " Nielson-Koster indexes is used for d shell identification in"
        WRITE (*,"(5A)")"   ",NAME(1:K-1)//'.lsj'//'.lbl', ", ",       &
                              NAME(1:K-1)//'.uni.lsj'//'.lbl',","
        WRITE (*,"(5A)")"   ",NAME(1:K-1)//'.uni.lsj'//'.sum',", and ",&
                              NAME(1:K-1)//'.lsj'//'.j',"  files"
      END IF
      WRITE (*,"(A)")                                                  &
      " while seniority notation for this shell is used in other files"
!GG-2024 end
      print *, " "
!
...
!GG NIST
      integer :: NCFMIN, NCFMAX, NCF_LS_jj_MAX
      ...
      do  IBLKNUM = 1, NBLOCK
         ...
         do  i = NCFMIN, NCFMAX
            ...
            do  lev = 1, number_of_levels(IBLKNUM)
                level = levels(IBLKNUM,lev)
                LOC_i8 = (level-1)*NCF_i8
                sumthrsh = sumthrsh + dabs(EVEC(i+LOC_i8))
            end do
...
!GG NIST
         ...
         do  lev = 1, number_of_levels(IBLKNUM)
            ...
            do  i = NCFMIN, NCFMAX
               if(ithresh(i) == 1) then
                  LOC_i8 = (level-1)*NCF_i8
                  wa = EVEC(i+LOC_i8) * EVEC(i+LOC_i8)
                  wb = wb + wa
                  do  j = 1,999
...
!CYC-2023 Corrections, fix the wrong output information.
...
!     output to *.lsj.lbl
            ...
            DO j = 1,nocsf_min
               ...
               IF(J == 1) THEN
                  write(57,'(I3,1X,A4,5X,A1,8X,F16.9,5X,F7.3,A)')        &
...
!                                                                      *
!     Calls: getchLS, packls_lbl, packls_j.                            *
!                                                                      *
!     Written by G. Gaigalas,                                          *
!     NIST                                     last update: Dec 2015   *
!                                                                      *
...
!-----------------------------------------------
      ...
      do j = csf_set_LS%nwcore+1, csf_set_LS%nwshells
         if (csf_set_LS%csf(csf_number)%occupation(j) > 0) then
            ...
            call getchLS_lbl(csf_number,j,LS,XLS)
...
!CYC-2023
! Modification for configurations with empty 1s subshell
               call setLS_job_count(isubc + 1, rez)
!CYC-2023 end
            end if
         else
...
!     Written by  G. Gaigalas                        Vilnius, Feb 2024 *
!                                                                      *
!***********************************************************************
!-----------------------------------------------
!   M o d u l e s
...
!     The maximum number of levels in the list
      INTEGER, PARAMETER :: Lev_No  = 10000
!     The maximum number of mixing coefficients of ASF expension
      INTEGER, PARAMETER :: Vec_No  = 10000
!-----------------------------------------------
!   L o c a l   V a r i a b l e s
!-----------------------------------------------
      CHARACTER(len=256), DIMENSION(:,:), POINTER :: COUPLING
      REAL(DOUBLE), DIMENSION(:,:), POINTER       :: COMP, MIX
      CHARACTER(len=3), DIMENSION(:), POINTER     :: Str_No
      CHARACTER(len=5), DIMENSION(:), POINTER     :: Str_J
      CHARACTER(len=1), DIMENSION(:), POINTER     :: Str_P
      CHARACTER(len=256), DIMENSION(:), POINTER   :: OPT_COUPLING
      CHARACTER(len=256), DIMENSION(:), POINTER   :: tmp_COUPLING
      REAL(DOUBLE), DIMENSION(:), POINTER         :: ENERGY, PRO
      REAL(DOUBLE), DIMENSION(:), POINTER         :: tmp_COMP, tmp_MIX
      INTEGER, DIMENSION(:), POINTER           :: MAS_MAX, ICOUNT, IPRGG
      CHARACTER    :: RECORD*53
      CHARACTER    :: FORM*11
      CHARACTER    :: Str*7
...
!-----------------------------------------------
      allocate(COUPLING(Vec_No,Lev_No))
      allocate(COMP(Vec_No,Lev_No))
      allocate(MIX(Vec_No,Lev_No))
      allocate(Str_No(Lev_No))
      allocate(Str_J(Lev_No))
      allocate(Str_P(Lev_No))
      allocate(tmp_COUPLING(Vec_No))
      allocate(OPT_COUPLING(Lev_No))
      allocate(ENERGY(Lev_No))
      allocate(PRO(Lev_No))
      allocate(tmp_COMP(Vec_No))
      allocate(tmp_MIX(Vec_No))
      allocate(MAS_MAX(Lev_No))
      allocate(ICOUNT(Lev_No))
      allocate(IPRGG(Lev_No))
!
      REWIND(57)
