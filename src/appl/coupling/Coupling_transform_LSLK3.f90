...
!***********************************************************************
!
      module Coupling_transform_LSLK3
!                                                                      *
...
!-----------------------------------------------
      ...
      use Coupling_data
      use Coupling_transform_cfg_LSLK3
!-----------------------------------------------
!   R o u t i n e s
!-----------------------------------------------
      public  :: main_LSLK3
...
!***********************************************************************
!                                                                      *
      subroutine main_LSLK3 (icoupling_nr_in_expansions,              &
...
!***********************************************************************
      ...
      integer::istate
      integer::icfg, icsf_LK3
      integer::nr_of_csfs_LK3, cfg_nr_of_csfs_LK3
      real(kind=dp):: sum_of_state
!
!      write(*,*) '  subroutine main_lslk3'
      write(iwrite_log,*) ''
      write(iwrite_log,*) 'LK3 coupling:'
      nr_of_LS_coupling = inr_of_LS_coupling
      if(nr_of_LS_coupling.lt.1) then
         write(*,*)                                                    &
           'ERROR in subroutine main_LSLK3: nr_of_LS_coupling.lt.1; ',&
...
!           call print_expansion_cfg_LS
            call count_nr_of_csfs_LK3(cfg_nr_of_csfs_LK3)
            nr_of_csfs_LK3 = nr_of_csfs_LK3 + cfg_nr_of_csfs_LK3
...
!     Fill up expansions_LK%expansions(istate)
         ...
         do icfg=1, cfg_structure_LS%nr_of_cfgs
            call form_expansion_cfg_LS(istate, icfg)
            call main_cfg_lslk3(print_level)
            do icsf_LK3=1, expansion_cfg_LK3%size, 1
               nr_of_csfs_LK3 = nr_of_csfs_LK3 + 1
               if(nr_of_csfs_LK3 .gt.                                 &
               all_expansions%coupling_expansions(                     &
               icoupling_nr_in_expansions)%expansions(istate)%size) then
                  write(*,*) 'STOP at subroutine main_lslk3  : ',     &
                             'nr_of_csfs_LK3.gt.',                    &
...
!unsing this I am SURE that in "b=a" "b" is "empty" csf ...
               all_expansions%coupling_expansions(                     &
                  icoupling_nr_in_expansions)%expansions(istate)%      &
                  csfs(nr_of_csfs_LK3)=expansion_cfg_LK3%              &
                  csfs(icsf_LK3)
               all_expansions%coupling_expansions(                     &
                  icoupling_nr_in_expansions)%expansions(istate)%      &
                  coeffs(nr_of_csfs_LK3)=expansion_cfg_LK3%            &
                  coeffs(icsf_LK3)
               sum_of_state = sum_of_state + all_expansions%           &
                  coupling_expansions(icoupling_nr_in_expansions)%     &
                  expansions(istate)%coeffs(nr_of_csfs_LK3)*           &
                  all_expansions%coupling_expansions(                  &
                  icoupling_nr_in_expansions)%expansions(istate)%      &
                  coeffs(nr_of_csfs_LK3)
            end do
            call delete_cfg_expansions
         end do
         if((sum_of_state-TWO_dp*dp_coeff_precision).gt.ONE_dp) then
            write(*,*)                                                 &
               'POSSIBLE ERROR at subroutine main_LSLK3: ',            &
         ...
         write(iwrite_log,*) istate,nr_of_csfs_LK3,'   ',sum_of_state
         call delete_cfg_structure
      end do
!      write(*,*) '  end subroutine main_lslk3'
!
      contains
!
!***********************************************************************
      ...
      if(istate.eq.0) then
         ...
         if(istate.ne.all_expansions%coupling_expansions(              &
            ...
            write(*,*) 'STOP at subroutine cfg_count module ',         &
               'Coupling_transform_LSLK3: istate.ne.',                &
...
!
      end module Coupling_transform_LSLK3
