...
!***********************************************************************
!
      module Coupling_transform_LSLK
!                                                                      *
...
!-----------------------------------------------
      ...
      use Coupling_data
      use Coupling_transform_cfg_LSLK
!-----------------------------------------------
!   R o u t i n e s
!-----------------------------------------------
      public  :: main_LSLK
...
!***********************************************************************
!                                                                      *
      subroutine main_LSLK (icoupling_nr_in_expansions,                &
...
!***********************************************************************
      ...
      integer::istate
      integer::icfg, icsf_LK
      integer::nr_of_csfs_LK, cfg_nr_of_csfs_LK
      real(kind=dp):: sum_of_state
!
!      write(*,*) '  subroutine main_lslk'
      write(iwrite_log,*) ''
      write(iwrite_log,*) 'LK coupling:'
      nr_of_LS_coupling = inr_of_LS_coupling
      if(nr_of_LS_coupling.lt.1) then
         write(*,*)                                                    &
           'ERROR in subroutine main_LSLK: nr_of_LS_coupling.lt.1; ',  &
...
!           call print_expansion_cfg_LS
            call count_nr_of_csfs_LK(cfg_nr_of_csfs_LK)
            nr_of_csfs_LK = nr_of_csfs_LK + cfg_nr_of_csfs_LK
...
!     Fill up expansions_LK%expansions(istate)
         ...
         do icfg=1, cfg_structure_LS%nr_of_cfgs
            call form_expansion_cfg_LS(istate, icfg)
            call main_cfg_lslk(print_level)
            do icsf_LK=1, expansion_cfg_LK%size, 1
               nr_of_csfs_LK = nr_of_csfs_LK + 1
               if(nr_of_csfs_LK .gt.                                   &
               all_expansions%coupling_expansions(                     &
...
!unsing this I am SURE that in "b=a" "b" is "empty" csf ...
               all_expansions%coupling_expansions(                     &
                  icoupling_nr_in_expansions)%expansions(istate)%      &
                  csfs(nr_of_csfs_LK) = expansion_cfg_LK%csfs(icsf_LK)
               all_expansions%coupling_expansions(                     &
                  icoupling_nr_in_expansions)%expansions(istate)%      &
                  coeffs(nr_of_csfs_LK)=expansion_cfg_LK%coeffs(icsf_LK)
               sum_of_state = sum_of_state + all_expansions%           &
                  coupling_expansions(icoupling_nr_in_expansions)%     &
                  expansions(istate)%coeffs(nr_of_csfs_LK)*            &
                  all_expansions%coupling_expansions(                  &
                  icoupling_nr_in_expansions)%expansions(istate)%      &
                  coeffs(nr_of_csfs_LK)
            end do
         ...
         write(iwrite_log,*) istate,nr_of_csfs_LK,'   ',sum_of_state
         call delete_cfg_structure
      end do
!      write(*,*) '  end subroutine main_lslk'
!
      contains
!
!***********************************************************************
      ...
      if(istate.eq.0) then
         ...
         if(istate.ne.all_expansions%coupling_expansions(              &
            ...
            write(*,*) 'STOP at subroutine cfg_count module ',         &
               'Coupling_transform_LSLK: istate.ne.',                  &
...
!
      end module Coupling_transform_LSLK
