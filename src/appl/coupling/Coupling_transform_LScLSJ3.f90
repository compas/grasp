...
!***********************************************************************
!
      module Coupling_transform_LScLSJ3
!                                                                      *
...
!-----------------------------------------------
      ...
      use Coupling_data
      use Coupling_transform_cfg_LScLSJ3
!-----------------------------------------------
!   R o u t i n e s
!-----------------------------------------------
      public  :: main_LScLSJ3
...
!***********************************************************************
!                                                                      *
      subroutine main_LScLSJ3 (icoupling_nr_in_expansions,             &
...
!***********************************************************************
      ...
      integer::istate
      integer::icfg, icsf_cLSJ3
      integer::nr_of_csfs_cLSJ3, cfg_nr_of_csfs_cLSJ3
      real(kind=dp):: sum_of_state
!
!      write(*,*) '  subroutine main_lsclsj3'
      write(iwrite_log,*) ''
      write(iwrite_log,*) 'cLSJ3 coupling:'
      nr_of_LS_coupling = inr_of_LS_coupling
      if(nr_of_LS_coupling.lt.1) then
         write(*,*)                                                    &
          'ERROR in subroutine main_LScLSJ3: nr_of_LS_coupling.lt.1; ',&
...
!     Define the size of mchf expansion in cLSJ3 coupling:
         nr_of_csfs_cLSJ3=0
         do icfg=1, cfg_structure_LS%nr_of_cfgs
            call form_expansion_cfg_LS(istate, icfg)
!           call print_expansion_cfg_LS
            call count_nr_of_csfs_cLSJ3(cfg_nr_of_csfs_cLSJ3)
            nr_of_csfs_cLSJ3 = nr_of_csfs_cLSJ3 + cfg_nr_of_csfs_cLSJ3
         ...
         allocate(all_expansions%coupling_expansions(                  &
            ...
            icoupling_nr_in_expansions)%expansions(istate)%size))
!
!     Fill up expansions_cLSJ3%expansions(istate)
         nr_of_csfs_cLSJ3=0
         sum_of_state = ZERO_dp
         do icfg=1, cfg_structure_LS%nr_of_cfgs
            call form_expansion_cfg_LS(istate, icfg)
            call main_cfg_lsclsj3(print_level)
            do icsf_cLSJ3=1, expansion_cfg_cLSJ3%size, 1
               nr_of_csfs_cLSJ3 = nr_of_csfs_cLSJ3 + 1
               if(nr_of_csfs_cLSJ3 .gt.                                &
               all_expansions%coupling_expansions(                     &
               icoupling_nr_in_expansions)%expansions(istate)%size) then
                  write(*,*) 'STOP at subroutine main_lsclsj3  : ',    &
                             'nr_of_csfs_cLSJ3.gt.',                   &
...
!unsing this I am SURE that in "b=a" "b" is "empty" csf ...
               all_expansions%coupling_expansions(                     &
                  icoupling_nr_in_expansions)%expansions(istate)%      &
                  csfs(nr_of_csfs_cLSJ3)=expansion_cfg_cLSJ3%          &
                  csfs(icsf_cLSJ3)
               all_expansions%coupling_expansions(                     &
                  icoupling_nr_in_expansions)%expansions(istate)%      &
                  coeffs(nr_of_csfs_cLSJ3)=expansion_cfg_cLSJ3%        &
                  coeffs(icsf_cLSJ3)
               sum_of_state = sum_of_state + all_expansions%           &
                  coupling_expansions(icoupling_nr_in_expansions)%     &
                  expansions(istate)%coeffs(nr_of_csfs_cLSJ3)*         &
                  all_expansions%coupling_expansions(                  &
                  icoupling_nr_in_expansions)%expansions(istate)%      &
                  coeffs(nr_of_csfs_cLSJ3)
            end do
            call delete_cfg_expansions
         end do
         if((sum_of_state-10*dp_coeff_precision).gt.ONE_dp) then
            write(*,*)                                                 &
               'POSSIBLE ERROR at subroutine main_LScLSJ3: ',          &
         ...
         write(iwrite_log,*) istate,nr_of_csfs_cLSJ3,'   ',sum_of_state
         call delete_cfg_structure
      end do
!      write(*,*) '  end subroutine main_lsclsj3'
!
      contains
!
!***********************************************************************
      ...
      if(istate.eq.0) then
         ...
         if(istate.ne.all_expansions%coupling_expansions(              &
            ...
            write(*,*) 'STOP at subroutine cfg_count module ',         &
               'Coupling_transform_LScLSJ3: istate.ne.',               &
...
!
      end module Coupling_transform_LScLSJ3
