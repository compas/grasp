...
!***********************************************************************
!
      module Coupling_transform_LSjj3
!                                                                      *
...
!-----------------------------------------------
      ...
      use Coupling_data
      use Coupling_transform_cfg_LSjj3
!-----------------------------------------------
!   R o u t i n e s
!-----------------------------------------------
      public  :: main_LSjj3
...
!***********************************************************************
!                                                                      *
      subroutine main_LSjj3 (icoupling_nr_in_expansions,             &
...
!***********************************************************************
      ...
      integer::istate
      integer::icfg, icsf_jj3
      integer::nr_of_csfs_jj3, cfg_nr_of_csfs_jj3
      real(kind=dp):: sum_of_state
!
!      write(*,*) '  subroutine main_lsjj3'
      write(iwrite_log,*) ''
      write(iwrite_log,*) 'jj3 coupling:'
      nr_of_LS_coupling = inr_of_LS_coupling
      if(nr_of_LS_coupling.lt.1) then
         write(*,*)                                                    &
          'ERROR in subroutine main_LSjj3: nr_of_LS_coupling.lt.1; ',&
...
!     Define the size of mchf expansion in jj3 coupling:
         nr_of_csfs_jj3=0
         do icfg=1, cfg_structure_LS%nr_of_cfgs
            call form_expansion_cfg_LS(istate, icfg)
!           call print_expansion_cfg_LS
            call count_nr_of_csfs_jj3(cfg_nr_of_csfs_jj3)
            nr_of_csfs_jj3 = nr_of_csfs_jj3 + cfg_nr_of_csfs_jj3
         ...
         allocate(all_expansions%coupling_expansions(                  &
            ...
            icoupling_nr_in_expansions)%expansions(istate)%size))
!
!     Fill up expansions_jj3%expansions(istate)
         nr_of_csfs_jj3=0
         sum_of_state = ZERO_dp
         do icfg=1, cfg_structure_LS%nr_of_cfgs
            call form_expansion_cfg_LS(istate, icfg)
            call main_cfg_lsjj3(print_level)
            do icsf_jj3=1, expansion_cfg_jj3%size, 1
               nr_of_csfs_jj3 = nr_of_csfs_jj3 + 1
               if(nr_of_csfs_jj3 .gt.                                &
               all_expansions%coupling_expansions(                     &
               icoupling_nr_in_expansions)%expansions(istate)%size) then
                  write(*,*) 'STOP at subroutine main_lsjj3  : ',    &
                             'nr_of_csfs_jj3.gt.',                   &
...
!unsing this I am SURE that in "b=a" "b" is "empty" csf ...
               all_expansions%coupling_expansions(                     &
                  icoupling_nr_in_expansions)%expansions(istate)%      &
                  csfs(nr_of_csfs_jj3)=expansion_cfg_jj3%          &
                  csfs(icsf_jj3)
               all_expansions%coupling_expansions(                     &
                  icoupling_nr_in_expansions)%expansions(istate)%      &
                  coeffs(nr_of_csfs_jj3)=expansion_cfg_jj3%        &
                  coeffs(icsf_jj3)
               sum_of_state = sum_of_state + all_expansions%           &
                  coupling_expansions(icoupling_nr_in_expansions)%     &
                  expansions(istate)%coeffs(nr_of_csfs_jj3)*         &
                  all_expansions%coupling_expansions(                  &
                  icoupling_nr_in_expansions)%expansions(istate)%      &
                  coeffs(nr_of_csfs_jj3)
            end do
            call delete_cfg_expansions
         end do
         if((sum_of_state-10*dp_coeff_precision).gt.ONE_dp) then
            write(*,*)                                                 &
               'POSSIBLE ERROR at subroutine main_LSjj3: ',          &
         ...
         write(iwrite_log,*) istate,nr_of_csfs_jj3,'   ',sum_of_state
         call delete_cfg_structure
      end do
!      write(*,*) '  end subroutine main_lsjj3'
!
      contains
!
!***********************************************************************
      ...
      if(istate.eq.0) then
         ...
         if(istate.ne.all_expansions%coupling_expansions(              &
            ...
            write(*,*) 'STOP at subroutine cfg_count module ',         &
               'Coupling_transform_LSjj3: istate.ne.',               &
...
!
      end module Coupling_transform_LSjj3
