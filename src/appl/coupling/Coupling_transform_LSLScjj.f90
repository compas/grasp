...
!***********************************************************************
!
      module Coupling_transform_LSLScjj
!                                                                      *
...
!-----------------------------------------------
      ...
      use Coupling_data
      use Coupling_transform_cfg_LSLScjj
!-----------------------------------------------
!   R o u t i n e s
!-----------------------------------------------
      public  :: main_LSLScjj
...
!***********************************************************************
!                                                                      *
      subroutine main_LSLScjj (icoupling_nr_in_expansions,                &
...
!***********************************************************************
      ...
      integer::istate
      integer::icfg, icsf_LScjj
      integer::nr_of_csfs_LScjj, cfg_nr_of_csfs_LScjj
      real(kind=dp):: sum_of_state
!
!      write(*,*) '  subroutine main_lslscjj'
      write(iwrite_log,*) ''
      write(iwrite_log,*) 'LScjj coupling:'
      nr_of_LS_coupling = inr_of_LS_coupling
      if(nr_of_LS_coupling.lt.1) then
         write(*,*)                                                    &
          'ERROR in subroutine main_LSLScjj: nr_of_LS_coupling.lt.1; ',&
...
!     Define the size of mchf expansion in LScjj coupling:
         nr_of_csfs_LScjj=0
         do icfg=1, cfg_structure_LS%nr_of_cfgs
            call form_expansion_cfg_LS(istate, icfg)
!           call print_expansion_cfg_LS
            call count_nr_of_csfs_LScjj(cfg_nr_of_csfs_LScjj)
            nr_of_csfs_LScjj = nr_of_csfs_LScjj + cfg_nr_of_csfs_LScjj
         ...
         allocate(all_expansions%coupling_expansions(                  &
            ...
            icoupling_nr_in_expansions)%expansions(istate)%size))
!
!     Fill up expansions_LScjj%expansions(istate)
         nr_of_csfs_LScjj=0
         sum_of_state = ZERO_dp
         do icfg=1, cfg_structure_LS%nr_of_cfgs
            call form_expansion_cfg_LS(istate, icfg)
            call main_cfg_lslscjj(print_level)
            do icsf_LScjj=1, expansion_cfg_LScjj%size, 1
               nr_of_csfs_LScjj = nr_of_csfs_LScjj + 1
               if(nr_of_csfs_LScjj .gt.                                &
               all_expansions%coupling_expansions(                     &
...
!unsing this I am SURE that in "b=a" "b" is "empty" csf ...
               all_expansions%coupling_expansions(                     &
                  icoupling_nr_in_expansions)%expansions(istate)%      &
                  csfs(nr_of_csfs_LScjj) = expansion_cfg_LScjj%csfs(icsf_LScjj)
               all_expansions%coupling_expansions(                     &
                  icoupling_nr_in_expansions)%expansions(istate)%      &
                  coeffs(nr_of_csfs_LScjj)=expansion_cfg_LScjj%coeffs(icsf_LScjj)
               sum_of_state = sum_of_state + all_expansions%           &
                  coupling_expansions(icoupling_nr_in_expansions)%     &
                  expansions(istate)%coeffs(nr_of_csfs_LScjj)*         &
                  all_expansions%coupling_expansions(                  &
                  icoupling_nr_in_expansions)%expansions(istate)%      &
                  coeffs(nr_of_csfs_LScjj)
            end do
            call delete_cfg_expansions
         end do
         if((sum_of_state-TWO_dp*dp_coeff_precision).gt.ONE_dp) then
            write(*,*)                                                 &
               'POSSIBLE ERROR at subroutine main_LSLScjj: ',             &
         ...
         write(iwrite_log,*) istate,nr_of_csfs_LScjj,'   ',sum_of_state
         call delete_cfg_structure
      end do
!      write(*,*) '  end subroutine main_lslscjj'
!
      contains
!
!***********************************************************************
      ...
      if(istate.eq.0) then
         ...
         if(istate.ne.all_expansions%coupling_expansions(              &
            ...
            write(*,*) 'STOP at subroutine cfg_count module ',         &
               'Coupling_transform_LSLScjj: istate.ne.',                  &
...
!
      end module Coupling_transform_LSLScjj
