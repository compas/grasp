...
!***********************************************************************
!
      module Coupling_transform_LSjj2
!                                                                      *
...
!-----------------------------------------------
      ...
      use Coupling_data
      use Coupling_transform_cfg_LSjj2
!-----------------------------------------------
!   R o u t i n e s
!-----------------------------------------------
      public  :: main_LSjj2
...
!***********************************************************************
!                                                                      *
      subroutine main_LSjj2 (icoupling_nr_in_expansions,                &
...
!***********************************************************************
      ...
      integer::istate
      integer::icfg, icsf_jj2
      integer::nr_of_csfs_jj2, cfg_nr_of_csfs_jj2
      real(kind=dp):: sum_of_state
!
!      write(*,*) '  subroutine main_lslsjj2'
      write(iwrite_log,*) ''
      write(iwrite_log,*) 'jj2 coupling:'
      nr_of_LS_coupling = inr_of_LS_coupling
      if(nr_of_LS_coupling.lt.1) then
         write(*,*)                                                    &
           'ERROR in subroutine main_LSjj2: nr_of_LS_coupling.lt.1; ',  &
...
!     Define the size of mchf expansion in jj2 coupling:
         nr_of_csfs_jj2=0
         do icfg=1, cfg_structure_LS%nr_of_cfgs
            call form_expansion_cfg_LS(istate, icfg)
!           call print_expansion_cfg_LS
            call count_nr_of_csfs_jj2(cfg_nr_of_csfs_jj2)
            nr_of_csfs_jj2 = nr_of_csfs_jj2 + cfg_nr_of_csfs_jj2
         ...
         allocate(all_expansions%coupling_expansions(                  &
            ...
            icoupling_nr_in_expansions)%expansions(istate)%size))
!
!     Fill up expansions_jj2%expansions(istate)
         nr_of_csfs_jj2=0
         sum_of_state = ZERO_dp
         do icfg=1, cfg_structure_LS%nr_of_cfgs
            call form_expansion_cfg_LS(istate, icfg)
            call main_cfg_lslsjj2(print_level)
            do icsf_jj2=1, expansion_cfg_jj2%size, 1
               nr_of_csfs_jj2 = nr_of_csfs_jj2 + 1
               if(nr_of_csfs_jj2 .gt.                                  &
               all_expansions%coupling_expansions(                     &
...
!unsing this I am SURE that in "b=a" "b" is "empty" csf ...
               all_expansions%coupling_expansions(                     &
                  icoupling_nr_in_expansions)%expansions(istate)%      &
                  csfs(nr_of_csfs_jj2) = expansion_cfg_jj2%csfs(icsf_jj2)
               all_expansions%coupling_expansions(                     &
                  icoupling_nr_in_expansions)%expansions(istate)%      &
                  coeffs(nr_of_csfs_jj2)=expansion_cfg_jj2%coeffs(icsf_jj2)
               sum_of_state = sum_of_state + all_expansions%           &
                  coupling_expansions(icoupling_nr_in_expansions)%     &
                  expansions(istate)%coeffs(nr_of_csfs_jj2)*           &
                  all_expansions%coupling_expansions(                  &
                  icoupling_nr_in_expansions)%expansions(istate)%      &
                  coeffs(nr_of_csfs_jj2)
            end do
            call delete_cfg_expansions
         end do
         if((sum_of_state-TWO_dp*dp_coeff_precision).gt.ONE_dp) then
            write(*,*)                                                 &
               'POSSIBLE ERROR at subroutine main_LSjj2: ',            &
         ...
         write(iwrite_log,*) istate,nr_of_csfs_jj2,'   ',sum_of_state
         call delete_cfg_structure
      end do
!      write(*,*) '  end subroutine main_lslsjj2'
!
      contains
!
!***********************************************************************
      ...
      if(istate.eq.0) then
         ...
         if(istate.ne.all_expansions%coupling_expansions(              &
            ...
            write(*,*) 'STOP at subroutine cfg_count module ',         &
               'Coupling_transform_LSjj2: istate.ne.',                  &
...
!
      end module Coupling_transform_LSjj2
