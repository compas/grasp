...
!-----------------------------------------------
      ...
      write(*,*)  
      write(*,*) 'Output files: clist.out, clist.log'
      write(*,*) 'Note that clist.out is not in block form. To convert'
      write(*,*) 'to block form copy clist.out to rcsf.inp and run'
      write(*,*) 'rcsfblock'
      write(*,*)  
      write (*, 200) '    *  : new list' 
      ...
      do while(.TRUE.) 
         call matbin (org, lock, closed, varmax, skal, second, anel, par, low, &
            nmax, lim, dubbel, minj, maxj) 
         if (.not.second) exit  
      ...
      if (mod(ii,2)==0 .and. ii/=0) call copy7t9 
      if (mod(ii,2) /= 0) then 
         open(unit=93, file='fil1.dat', status='unknown', position='asis') 
         close(unit=93, status='delete') 
      endif 
      if (ii == 0) close(unit=7, status='delete') 
      stop  
  100 format(a) 
  200 format(' ',10a) 
  300 format(' ',a,i2,a) 
