...
!-----------------------------------------------
      ...
      integer :: an80, an81, an82, an83, an84, an85, an86, an87, an90, an91, &
         ...
         , plusa4, plusa5, plusa6, plusa7, plusa8, plusa9, par0, par, i, j, &
         antal, dum, plusba, plusca, plusda, plusea, plusfa, plusb1, plusb2, &
         plusb3, plusb4, plusb5, plusb6, plusb7, plusb8, plusb9, plusc1, plusc2&
         , plusc3, plusc4, plusc5, plusc6, plusc7, plusc8, plusc9, plusd1, &
         plusd2, plusd3, plusd4, plusd5, plusd6, plusd7, plusd8, plusd9, pluse1&
         , pluse2, pluse3, pluse4, pluse5, pluse6, pluse7, pluse8, pluse9, &
         plusf1, plusf2, plusf3, plusf4, plusf5, plusf6, plusf7, plusf8, plusf9&
         , anba, anca, anda, anea, anfa, anb0, anb1, anb2, anb3, anb4, anb5, &
         anb6, anb7, anb8, anb9, anc0, anc1, anc2, anc3, anc4, anc5, anc6, anc7&
         , anc8, anc9, and0, and1, and2, and3, and4, and5, and6, and7, and8, &
         and9, ane0, ane1, ane2, ane3, ane4, ane5, ane6, ane7, ane8, ane9, anf0&
         , anf1, anf2, anf3, anf4, anf5, anf6, anf7, anf8, anf9 
      real :: elar 
      logical :: first, finns 
!-----------------------------------------------
!
...
!     1s
      call sluggo (1, 0, varmax, varupp, varned, ansats, org, lock(1,0), low, &
         start(1,0), stopp(1,0)) 
      do an10 = start(1,0), stopp(1,0), -1 
         antel(1,0) = an10 
         if (antel(1,0)>antal .or. antel(1,0)<lim(1)) cycle  
         ansats(1,0,0) = an10 
!     2s
         call sluggo (2, 0, varmax, varupp, varned, ansats, org, lock(2,0), low&
            , start(2,0), stopp(2,0)) 
         do an20 = start(2,0), stopp(2,0), -1 
            antel(2,0) = an20 + antel(1,0) 
            if (antel(2,0) > antal) cycle  
            ansats(2,0,0) = an20 
!     2p
            call sluggo (2, 1, varmax, varupp, varned, ansats, org, lock(2,1), &
               low, start(2,1), stopp(2,1)) 
            do an21 = start(2,1), stopp(2,1), -1 
...
!     3s
                  call sluggo (3, 0, varmax, varupp, varned, ansats, org, lock(&
                     3,0), low, start(3,0), stopp(3,0)) 
                  do an30 = start(3,0), stopp(3,0), -1 
                     antel(3,0) = an30 + antel(2,1) 
                     if (antel(3,0) > antal) cycle  
                     ansats(3,0,0) = an30 
!     3p
                     call sluggo (3, 1, varmax, varupp, varned, ansats, org, &
                        lock(3,1), low, start(3,1), stopp(3,1)) 
                     do an31 = start(3,1), stopp(3,1), -1 
...
!     3d
                           call sluggo (3, 2, varmax, varupp, varned, ansats, &
                              org, lock(3,2), low, start(3,2), stopp(3,2)) 
                           do an32 = start(3,2), stopp(3,2), -1 
...
!     4s
                                 call sluggo (4, 0, varmax, varupp, varned, &
                                    ansats, org, lock(4,0), low, start(4,0), &
                                    stopp(4,0)) 
                                 do an40 = start(4,0), stopp(4,0), -1 
                                    antel(4,0) = an40 + antel(3,2) 
                                    if (antel(4,0) > antal) cycle  
                                    ansats(4,0,0) = an40 
!     4p
                                    call sluggo (4, 1, varmax, varupp, varned, &
                                       ansats, org, lock(4,1), low, start(4,1)&
                                       , stopp(4,1)) 
                                    do an41 = start(4,1), stopp(4,1), -1 
...
!     4d
                                    call sluggo (4, 2, varmax, varupp, varned, &
                                       ansats, org, lock(4,2), low, start(4,2)&
                                       , stopp(4,2)) 
                                    do an42 = start(4,2), stopp(4,2), -1 
...
!     4f
                                    call sluggo (4, 3, varmax, varupp, varned, &
                                       ansats, org, lock(4,3), low, start(4,3)&
                                       , stopp(4,3)) 
                                    do an43 = start(4,3), stopp(4,3), -1 
...
!     5s
                                    call sluggo (5, 0, varmax, varupp, varned, &
                                       ansats, org, lock(5,0), low, start(5,0)&
                                       , stopp(5,0)) 
                                    do an50 = start(5,0), stopp(5,0), -1 
                                    antel(5,0) = an50 + antel(4,3) 
                                    if (antel(5,0) > antal) cycle  
                                    ansats(5,0,0) = an50 
!     5p
                                    call sluggo (5, 1, varmax, varupp, varned, &
                                       ansats, org, lock(5,1), low, start(5,1)&
                                       , stopp(5,1)) 
                                    do an51 = start(5,1), stopp(5,1), -1 
...
!     5d
                                    call sluggo (5, 2, varmax, varupp, varned, &
                                       ansats, org, lock(5,2), low, start(5,2)&
                                       , stopp(5,2)) 
                                    do an52 = start(5,2), stopp(5,2), -1 
...
!     5f
                                    call sluggo (5, 3, varmax, varupp, varned, &
                                       ansats, org, lock(5,3), low, start(5,3)&
                                       , stopp(5,3)) 
                                    do an53 = start(5,3), stopp(5,3), -1 
...
!     5g
                                    call sluggo (5, 4, varmax, varupp, varned, &
                                       ansats, org, lock(5,4), low, start(5,4)&
                                       , stopp(5,4)) 
                                    do an54 = start(5,4), stopp(5,4), -1 
...
!     6s
                                    call sluggo (6, 0, varmax, varupp, varned, &
                                       ansats, org, lock(6,0), low, start(6,0)&
                                       , stopp(6,0)) 
                                    do an60 = start(6,0), stopp(6,0), -1 
                                    antel(6,0) = an60 + antel(5,4) 
                                    if (antel(6,0)>antal .or. ansats(5,4,1)>2) &
                                       cycle  
                                    ansats(6,0,0) = an60 
!     6p
                                    call sluggo (6, 1, varmax, varupp, varned, &
                                       ansats, org, lock(6,1), low, start(6,1)&
                                       , stopp(6,1)) 
                                    do an61 = start(6,1), stopp(6,1), -1 
...
!     6d
                                    call sluggo (6, 2, varmax, varupp, varned, &
                                       ansats, org, lock(6,2), low, start(6,2)&
                                       , stopp(6,2)) 
                                    do an62 = start(6,2), stopp(6,2), -1 
...
!     6f
                                    call sluggo (6, 3, varmax, varupp, varned, &
                                       ansats, org, lock(6,3), low, start(6,3)&
                                       , stopp(6,3)) 
                                    do an63 = start(6,3), stopp(6,3), -1 
...
!     6g
                                    call sluggo (6, 4, varmax, varupp, varned, &
                                       ansats, org, lock(6,4), low, start(6,4)&
                                       , stopp(6,4)) 
                                    do an64 = start(6,4), stopp(6,4), -1 
...
!     6h
                                    call sluggo (6, 5, varmax, varupp, varned, &
                                       ansats, org, lock(6,5), low, start(6,5)&
                                       , stopp(6,5)) 
                                    do an65 = start(6,5), stopp(6,5), -1 
...
!     7s
                                    call sluggo (7, 0, varmax, varupp, varned, &
                                       ansats, org, lock(7,0), low, start(7,0)&
                                       , stopp(7,0)) 
                                    do an70 = start(7,0), stopp(7,0), -1 
                                    antel(7,0) = an70 + antel(6,5) 
                                    if (.not.(antel(7,0)<=antal .and. ansats(6,&
                                       5,1)<=2 .and. ansats(6,5,0)<=2)) cycle  
                                    ansats(7,0,0) = an70 
!     7p
                                    call sluggo (7, 1, varmax, varupp, varned, &
                                       ansats, org, lock(7,1), low, start(7,1)&
                                       , stopp(7,1)) 
                                    do an71 = start(7,1), stopp(7,1), -1 
...
!     7d
                                    call sluggo (7, 2, varmax, varupp, varned, &
                                       ansats, org, lock(7,2), low, start(7,2)&
                                       , stopp(7,2)) 
                                    do an72 = start(7,2), stopp(7,2), -1 
...
!     7f
                                    call sluggo (7, 3, varmax, varupp, varned, &
                                       ansats, org, lock(7,3), low, start(7,3)&
                                       , stopp(7,3)) 
                                    do an73 = start(7,3), stopp(7,3), -1 
...
!     7g
                                    call sluggo (7, 4, varmax, varupp, varned, &
                                       ansats, org, lock(7,4), low, start(7,4)&
                                       , stopp(7,4)) 
                                    do an74 = start(7,4), stopp(7,4), -1 
...
!     7h
                                    call sluggo (7, 5, varmax, varupp, varned, &
                                       ansats, org, lock(7,5), low, start(7,5)&
                                       , stopp(7,5)) 
                                    do an75 = start(7,5), stopp(7,5), -1 
...
!     7i
                                    call sluggo (7, 6, varmax, varupp, varned, &
                                       ansats, org, lock(7,6), low, start(7,6)&
                                       , stopp(7,6)) 
                                    do an76 = start(7,6), stopp(7,6), -1 
...
!     8s
                                    call sluggo (8, 0, varmax, varupp, varned, &
                                       ansats, org, lock(8,0), low, start(8,0)&
                                       , stopp(8,0)) 
                                    do an80 = start(8,0), stopp(8,0), -1 
                                    antel(8,0) = an80 + antel(7,6) 
                                    if (.not.(antel(8,0)<=antal .and. ansats(7,&
                                       6,1)<=2 .and. ansats(7,6,0)<=2)) cycle  
                                    ansats(8,0,0) = an80 
!     8p
                                    call sluggo (8, 1, varmax, varupp, varned, &
                                       ansats, org, lock(8,1), low, start(8,1)&
                                       , stopp(8,1)) 
                                    do an81 = start(8,1), stopp(8,1), -1 
...
!     8d
                                    call sluggo (8, 2, varmax, varupp, varned, &
                                       ansats, org, lock(8,2), low, start(8,2)&
                                       , stopp(8,2)) 
                                    do an82 = start(8,2), stopp(8,2), -1 
...
!     8f
                                    call sluggo (8, 3, varmax, varupp, varned, &
                                       ansats, org, lock(8,3), low, start(8,3)&
                                       , stopp(8,3)) 
                                    do an83 = start(8,3), stopp(8,3), -1 
...
!     8g
                                    call sluggo (8, 4, varmax, varupp, varned, &
                                       ansats, org, lock(8,4), low, start(8,4)&
                                       , stopp(8,4)) 
 
                                    do an84 = start(8,4), stopp(8,4), -1 
...
!     8h
                                    call sluggo (8, 5, varmax, varupp, varned, &
                                       ansats, org, lock(8,5), low, start(8,5)&
                                       , stopp(8,5)) 
                                    do an85 = start(8,5), stopp(8,5), -1 
...
!     8i
                                    call sluggo (8, 6, varmax, varupp, varned, &
                                       ansats, org, lock(8,6), low, start(8,6)&
                                       , stopp(8,6)) 
                                    do an86 = start(8,6), stopp(8,6), -1 
...
!     8k
                                    call sluggo (8, 7, varmax, varupp, varned, &
                                       ansats, org, lock(8,7), low, start(8,7)&
                                       , stopp(8,7)) 
                                    do an87 = start(8,7), stopp(8,7), -1 
...
!     9s
                                    call sluggo (9, 0, varmax, varupp, varned, &
                                       ansats, org, lock(9,0), low, start(9,0)&
                                       , stopp(9,0)) 
                                    do an90 = start(9,0), stopp(9,0), -1 
                                    antel(9,0) = an90 + antel(8,7) 
                                    if (.not.(antel(9,0)<=antal .and. ansats(8,&
                                       7,1)<=2 .and. ansats(8,7,0)<=2)) cycle  
                                    ansats(9,0,0) = an90 
!     9p
                                    call sluggo (9, 1, varmax, varupp, varned, &
                                       ansats, org, lock(9,1), low, start(9,1)&
                                       , stopp(9,1)) 
                                    do an91 = start(9,1), stopp(9,1), -1 
...
!     9d
                                    call sluggo (9, 2, varmax, varupp, varned, &
                                       ansats, org, lock(9,2), low, start(9,2)&
                                       , stopp(9,2)) 
                                    do an92 = start(9,2), stopp(9,2), -1 
...
!     9f
                                    call sluggo (9, 3, varmax, varupp, varned, &
                                       ansats, org, lock(9,3), low, start(9,3)&
                                       , stopp(9,3)) 
                                    do an93 = start(9,3), stopp(9,3), -1 
...
!     9g
                                    call sluggo (9, 4, varmax, varupp, varned, &
                                       ansats, org, lock(9,4), low, start(9,4)&
                                       , stopp(9,4)) 
                                    do an94 = start(9,4), stopp(9,4), -1 
...
!     9h
                                    call sluggo (9, 5, varmax, varupp, varned, &
                                       ansats, org, lock(9,5), low, start(9,5)&
                                       , stopp(9,5)) 
                                    do an95 = start(9,5), stopp(9,5), -1 
...
!     9i
                                    call sluggo (9, 6, varmax, varupp, varned, &
                                       ansats, org, lock(9,6), low, start(9,6)&
                                       , stopp(9,6)) 
                                    do an96 = start(9,6), stopp(9,6), -1 
...
!     9k
                                    call sluggo (9, 7, varmax, varupp, varned, &
                                       ansats, org, lock(9,7), low, start(9,7)&
                                       , stopp(9,7)) 
                                    do an97 = start(9,7), stopp(9,7), -1 
...
!     9l
                                    call sluggo (9, 8, varmax, varupp, varned, &
                                       ansats, org, lock(9,8), low, start(9,8)&
                                       , stopp(9,8)) 
                                    do an98 = start(9,8), stopp(9,8), -1 
...
!     10s
                                    call sluggo (10, 0, varmax, varupp, varned&
                                       , ansats, org, lock(10,0), low, start(10&
                                       ,0), stopp(10,0)) 
                                    do ana0 = start(10,0), stopp(10,0), -1 
                                    antel(10,0) = ana0 + antel(9,8) 
                                    if (.not.(antel(10,0)<=antal .and. ansats(9&
                                       ,8,1)<=2 .and. ansats(9,8,0)<=2)) cycle  
                                    ansats(10,0,0) = ana0 
!     10p
                                    call sluggo (10, 1, varmax, varupp, varned&
                                       , ansats, org, lock(10,1), low, start(10&
                                       ,1), stopp(10,1)) 
                                    do ana1 = start(10,1), stopp(10,1), -1 
...
!     10d
                                    call sluggo (10, 2, varmax, varupp, varned&
                                       , ansats, org, lock(10,2), low, start(10&
                                       ,2), stopp(10,2)) 
                                    do ana2 = start(10,2), stopp(10,2), -1 
...
!     10f
                                    call sluggo (10, 3, varmax, varupp, varned&
                                       , ansats, org, lock(10,3), low, start(10&
                                       ,3), stopp(10,3)) 
                                    do ana3 = start(10,3), stopp(10,3), -1 
...
!     10g
                                    call sluggo (10, 4, varmax, varupp, varned&
                                       , ansats, org, lock(10,4), low, start(10&
                                       ,4), stopp(10,4)) 
                                    do ana4 = start(10,4), stopp(10,4), -1 
...
!     10h
                                    call sluggo (10, 5, varmax, varupp, varned&
                                       , ansats, org, lock(10,5), low, start(10&
                                       ,5), stopp(10,5)) 
                                    do ana5 = start(10,5), stopp(10,5), -1 
...
!     10i
                                    call sluggo (10, 6, varmax, varupp, varned&
                                       , ansats, org, lock(10,6), low, start(10&
                                       ,6), stopp(10,6)) 
                                    do ana6 = start(10,6), stopp(10,6), -1 
...
!     10k
                                    call sluggo (10, 7, varmax, varupp, varned&
                                       , ansats, org, lock(10,7), low, start(10&
                                       ,7), stopp(10,7)) 
                                    do ana7 = start(10,7), stopp(10,7), -1 
...
!     10l
                                    call sluggo (10, 8, varmax, varupp, varned&
                                       , ansats, org, lock(10,8), low, start(10&
                                       ,8), stopp(10,8)) 
                                    do ana8 = start(10,8), stopp(10,8), -1 
...
!     10m
                                    call sluggo (10, 9, varmax, varupp, varned&
                                       , ansats, org, lock(10,9), low, start(10&
                                       ,9), stopp(10,9)) 
                                    do ana9 = start(10,9), stopp(10,9), -1 
...
!     11s
                                    call sluggo (11, 0, varmax, varupp, varned&
                                       , ansats, org, lock(11,0), low, start(11&
                                       ,0), stopp(11,0)) 
                                    do anb0 = start(11,0), stopp(11,0), -1 
                                    ...
                                    ansats(11,0,0) = anb0 
!     11p
                                    call sluggo (11, 1, varmax, varupp, varned&
                                       , ansats, org, lock(11,1), low, start(11&
                                       ,1), stopp(11,1)) 
                                    do anb1 = start(11,1), stopp(11,1), -1 
...
!     11d
                                    call sluggo (11, 2, varmax, varupp, varned&
                                       , ansats, org, lock(11,2), low, start(11&
                                       ,2), stopp(11,2)) 
                                    do anb2 = start(11,2), stopp(11,2), -1 
...
!     11f
                                    call sluggo (11, 3, varmax, varupp, varned&
                                       , ansats, org, lock(11,3), low, start(11&
                                       ,3), stopp(11,3)) 
                                    do anb3 = start(11,3), stopp(11,3), -1 
...
!     11g
                                    call sluggo (11, 4, varmax, varupp, varned&
                                       , ansats, org, lock(11,4), low, start(11&
                                       ,4), stopp(11,4)) 
                                    do anb4 = start(11,4), stopp(11,4), -1 
...
!     11h
                                    call sluggo (11, 5, varmax, varupp, varned&
                                       , ansats, org, lock(11,5), low, start(11&
                                       ,5), stopp(11,5)) 
                                    do anb5 = start(11,5), stopp(11,5), -1 
...
!     11i
                                    call sluggo (11, 6, varmax, varupp, varned&
                                       , ansats, org, lock(11,6), low, start(11&
                                       ,6), stopp(11,6)) 
                                    do anb6 = start(11,6), stopp(11,6), -1 
...
!     11k
                                    call sluggo (11, 7, varmax, varupp, varned&
                                       , ansats, org, lock(11,7), low, start(11&
                                       ,7), stopp(11,7)) 
                                    do anb7 = start(11,7), stopp(11,7), -1 
...
!     11l
                                    call sluggo (11, 8, varmax, varupp, varned&
                                       , ansats, org, lock(11,8), low, start(11&
                                       ,8), stopp(11,8)) 
                                    do anb8 = start(11,8), stopp(11,8), -1 
...
!     11m
                                    call sluggo (11, 9, varmax, varupp, varned&
                                       , ansats, org, lock(11,9), low, start(11&
                                       ,9), stopp(11,9)) 
                                    do anb9 = start(11,9), stopp(11,9), -1 
...
!     11n
                                    call sluggo (11, 10, varmax, varupp, varned&
                                       , ansats, org, lock(11,10), low, start(&
                                       11,10), stopp(11,10)) 
                                    do anba = start(11,10), stopp(11,10), -1 
...
!     12s
                                    call sluggo (12, 0, varmax, varupp, varned&
                                       , ansats, org, lock(12,0), low, start(12&
                                       ,0), stopp(12,0)) 
                                    do anc0 = start(12,0), stopp(12,0), -1 
                                    ...
                                    ansats(12,0,0) = anc0 
!     12p
                                    call sluggo (12, 1, varmax, varupp, varned&
                                       , ansats, org, lock(12,1), low, start(12&
                                       ,1), stopp(12,1)) 
                                    do anc1 = start(12,1), stopp(12,1), -1 
...
!     12d
                                    call sluggo (12, 2, varmax, varupp, varned&
                                       , ansats, org, lock(12,2), low, start(12&
                                       ,2), stopp(12,2)) 
                                    do anc2 = start(12,2), stopp(12,2), -1 
...
!     12f
                                    call sluggo (12, 3, varmax, varupp, varned&
                                       , ansats, org, lock(12,3), low, start(12&
                                       ,3), stopp(12,3)) 
                                    do anc3 = start(12,3), stopp(12,3), -1 
...
!     12g
                                    call sluggo (12, 4, varmax, varupp, varned&
                                       , ansats, org, lock(12,4), low, start(12&
                                       ,4), stopp(12,4)) 
                                    do anc4 = start(12,4), stopp(12,4), -1 
...
!     12h
                                    call sluggo (12, 5, varmax, varupp, varned&
                                       , ansats, org, lock(12,5), low, start(12&
                                       ,5), stopp(12,5)) 
                                    do anc5 = start(12,5), stopp(12,5), -1 
...
!     12i
                                    call sluggo (12, 6, varmax, varupp, varned&
                                       , ansats, org, lock(12,6), low, start(12&
                                       ,6), stopp(12,6)) 
                                    do anc6 = start(12,6), stopp(12,6), -1 
...
!     12k
                                    call sluggo (12, 7, varmax, varupp, varned&
                                       , ansats, org, lock(12,7), low, start(12&
                                       ,7), stopp(12,7)) 
                                    do anc7 = start(12,7), stopp(12,7), -1 
...
!     12l
                                    call sluggo (12, 8, varmax, varupp, varned&
                                       , ansats, org, lock(12,8), low, start(12&
                                       ,8), stopp(12,8)) 
                                    do anc8 = start(12,8), stopp(12,8), -1 
...
!     12m
                                    call sluggo (12, 9, varmax, varupp, varned&
                                       , ansats, org, lock(12,9), low, start(12&
                                       ,9), stopp(12,9)) 
                                    do anc9 = start(12,9), stopp(12,9), -1 
...
!     12n
                                    call sluggo (12, 10, varmax, varupp, varned&
                                       , ansats, org, lock(12,10), low, start(&
                                       12,10), stopp(12,10)) 
                                    do anca = start(12,10), stopp(12,10), -1 
...
!     13s
                                    call sluggo (13, 0, varmax, varupp, varned&
                                       , ansats, org, lock(13,0), low, start(13&
                                       ,0), stopp(13,0)) 
                                    do and0 = start(13,0), stopp(13,0), -1 
                                    ...
                                    ansats(13,0,0) = and0 
!     13p
                                    call sluggo (13, 1, varmax, varupp, varned&
                                       , ansats, org, lock(13,1), low, start(13&
                                       ,1), stopp(13,1)) 
                                    do and1 = start(13,1), stopp(13,1), -1 
...
!     13d
                                    call sluggo (13, 2, varmax, varupp, varned&
                                       , ansats, org, lock(13,2), low, start(13&
                                       ,2), stopp(13,2)) 
                                    do and2 = start(13,2), stopp(13,2), -1 
...
!     13f
                                    call sluggo (13, 3, varmax, varupp, varned&
                                       , ansats, org, lock(13,3), low, start(13&
                                       ,3), stopp(13,3)) 
                                    do and3 = start(13,3), stopp(13,3), -1 
...
!     13g
                                    call sluggo (13, 4, varmax, varupp, varned&
                                       , ansats, org, lock(13,4), low, start(13&
                                       ,4), stopp(13,4)) 
                                    do and4 = start(13,4), stopp(13,4), -1 
...
!     13h
                                    call sluggo (13, 5, varmax, varupp, varned&
                                       , ansats, org, lock(13,5), low, start(13&
                                       ,5), stopp(13,5)) 
                                    do and5 = start(13,5), stopp(13,5), -1 
...
!     13i
                                    call sluggo (13, 6, varmax, varupp, varned&
                                       , ansats, org, lock(13,6), low, start(13&
                                       ,6), stopp(13,6)) 
                                    do and6 = start(13,6), stopp(13,6), -1 
...
!     13k
                                    call sluggo (13, 7, varmax, varupp, varned&
                                       , ansats, org, lock(13,7), low, start(13&
                                       ,7), stopp(13,7)) 
                                    do and7 = start(13,7), stopp(13,7), -1 
...
!     13l
                                    call sluggo (13, 8, varmax, varupp, varned&
                                       , ansats, org, lock(13,8), low, start(13&
                                       ,8), stopp(13,8)) 
                                    do and8 = start(13,8), stopp(13,8), -1 
...
!     13m
                                    call sluggo (13, 9, varmax, varupp, varned&
                                       , ansats, org, lock(13,9), low, start(13&
                                       ,9), stopp(13,9)) 
                                    do and9 = start(13,9), stopp(13,9), -1 
...
!     13n
                                    call sluggo (13, 10, varmax, varupp, varned&
                                       , ansats, org, lock(13,10), low, start(&
                                       13,10), stopp(13,10)) 
                                    do anda = start(13,10), stopp(13,10), -1 
...
!     14s
                                    call sluggo (14, 0, varmax, varupp, varned&
                                       , ansats, org, lock(14,0), low, start(14&
                                       ,0), stopp(14,0)) 
                                    do ane0 = start(14,0), stopp(14,0), -1 
                                    ...
                                    ansats(14,0,0) = ane0 
!     14p
                                    call sluggo (14, 1, varmax, varupp, varned&
                                       , ansats, org, lock(14,1), low, start(14&
                                       ,1), stopp(14,1)) 
                                    do ane1 = start(14,1), stopp(14,1), -1 
...
!     14d
                                    call sluggo (14, 2, varmax, varupp, varned&
                                       , ansats, org, lock(14,2), low, start(14&
                                       ,2), stopp(14,2)) 
                                    do ane2 = start(14,2), stopp(14,2), -1 
...
!     14f
                                    call sluggo (14, 3, varmax, varupp, varned&
                                       , ansats, org, lock(14,3), low, start(14&
                                       ,3), stopp(14,3)) 
                                    do ane3 = start(14,3), stopp(14,3), -1 
...
!     14g
                                    call sluggo (14, 4, varmax, varupp, varned&
                                       , ansats, org, lock(14,4), low, start(14&
                                       ,4), stopp(14,4)) 
                                    do ane4 = start(14,4), stopp(14,4), -1 
...
!     14h
                                    call sluggo (14, 5, varmax, varupp, varned&
                                       , ansats, org, lock(14,5), low, start(14&
                                       ,5), stopp(14,5)) 
                                    do ane5 = start(14,5), stopp(14,5), -1 
...
!     14i
                                    call sluggo (14, 6, varmax, varupp, varned&
                                       , ansats, org, lock(14,6), low, start(14&
                                       ,6), stopp(14,6)) 
                                    do ane6 = start(14,6), stopp(14,6), -1 
...
!     14k
                                    call sluggo (14, 7, varmax, varupp, varned&
                                       , ansats, org, lock(14,7), low, start(14&
                                       ,7), stopp(14,7)) 
                                    do ane7 = start(14,7), stopp(14,7), -1 
...
!     14l
                                    call sluggo (14, 8, varmax, varupp, varned&
                                       , ansats, org, lock(14,8), low, start(14&
                                       ,8), stopp(14,8)) 
                                    do ane8 = start(14,8), stopp(14,8), -1 
...
!     14m
                                    call sluggo (14, 9, varmax, varupp, varned&
                                       , ansats, org, lock(14,9), low, start(14&
                                       ,9), stopp(14,9)) 
                                    do ane9 = start(14,9), stopp(14,9), -1 
...
!     14n
                                    call sluggo (14, 10, varmax, varupp, varned&
                                       , ansats, org, lock(14,10), low, start(&
                                       14,10), stopp(14,10)) 
                                    do anea = start(14,10), stopp(14,10), -1 
...
!     15s
                                    call sluggo (15, 0, varmax, varupp, varned&
                                       , ansats, org, lock(15,0), low, start(15&
                                       ,0), stopp(15,0)) 
                                    do anf0 = start(15,0), stopp(15,0), -1 
                                    ...
                                    ansats(15,0,0) = anf0 
!     15p
                                    call sluggo (15, 1, varmax, varupp, varned&
                                       , ansats, org, lock(15,1), low, start(15&
                                       ,1), stopp(15,1)) 
                                    do anf1 = start(15,1), stopp(15,1), -1 
...
!     15d
                                    call sluggo (15, 2, varmax, varupp, varned&
                                       , ansats, org, lock(15,2), low, start(15&
                                       ,2), stopp(15,2)) 
                                    do anf2 = start(15,2), stopp(15,2), -1 
...
!     15f
                                    call sluggo (15, 3, varmax, varupp, varned&
                                       , ansats, org, lock(15,3), low, start(15&
                                       ,3), stopp(15,3)) 
                                    do anf3 = start(15,3), stopp(15,3), -1 
...
!     15g
                                    call sluggo (15, 4, varmax, varupp, varned&
                                       , ansats, org, lock(15,4), low, start(15&
                                       ,4), stopp(15,4)) 
                                    do anf4 = start(15,4), stopp(15,4), -1 
...
!     15h
                                    call sluggo (15, 5, varmax, varupp, varned&
                                       , ansats, org, lock(15,5), low, start(15&
                                       ,5), stopp(15,5)) 
                                    do anf5 = start(15,5), stopp(15,5), -1 
...
!     15i
                                    call sluggo (15, 6, varmax, varupp, varned&
                                       , ansats, org, lock(15,6), low, start(15&
                                       ,6), stopp(15,6)) 
                                    do anf6 = start(15,6), stopp(15,6), -1 
...
!     15k
                                    call sluggo (15, 7, varmax, varupp, varned&
                                       , ansats, org, lock(15,7), low, start(15&
                                       ,7), stopp(15,7)) 
                                    do anf7 = start(15,7), stopp(15,7), -1 
...
!     15l
                                    call sluggo (15, 8, varmax, varupp, varned&
                                       , ansats, org, lock(15,8), low, start(15&
                                       ,8), stopp(15,8)) 
                                    do anf8 = start(15,8), stopp(15,8), -1 
...
!     15m
                                    call sluggo (15, 9, varmax, varupp, varned&
                                       , ansats, org, lock(15,9), low, start(15&
                                       ,9), stopp(15,9)) 
                                    do anf9 = start(15,9), stopp(15,9), -1 
...
!     15n
                                    call sluggo (15, 10, varmax, varupp, varned&
                                       , ansats, org, lock(15,10), low, start(&
                                       15,10), stopp(15,10)) 
                                    do anfa = start(15,10), stopp(15,10), -1 
                                    ...
                                    do i = 1, nmax 
                                    do j = 0, min(10,i - 1) 
                                    elar = elar + (ansats(i,j,0)+ansats(i,j,1)) 
                                    par = mod(par + j*(ansats(i,j,0)+ansats(i,j&
                                       ,1)),2) 
                                    end do 
                                    end do 
                                    if (par /= par0) cycle  
                                    if (elar /= antal) write (*, *) 'FEL' 
                                    cf = cf + 1 
                                    do i = 1, 15 
                                    write (fil, 5000) (ansats(i,j,0),j=0,min(10&
                                       ,i - 1)) 
                                    write (fil, 5000) (ansats(i,j,1),j=0,min(10&
                                       ,i - 1)) 
                                    end do 
                                    end do 
                                    end do 
